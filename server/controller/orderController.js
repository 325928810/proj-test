...
const getAllOrders = async (req, res) => {
  ...
  // if (!orders?.length) {
  //     return res.status(400).json({ message: "there are no orsers!!" })
  // }
  res.json(orders);
};
const getUserOrders = async (req, res) => {
  ...
  // if (!orders?.length) {
  //     return res.json({ message: "Your basket is already empty" })
  // }
  res.json(orders);
};
...
module.exports = { getAllOrders, createOrder, deleteOrder, getUserOrders };
